...
class MarkovChain:
	...
	def __init__(self, chainPath, level = 2, wb = False):
		"""chainPath is the location of the Markov chain to load; wb is for Shelve's
		writeback option. If writeback is set to true, Shelve will keep track of all
		changes to the chains and save them when closed. We don't need this during
		normal operation."""
		self.level = level
	...
	def getNextWords(self, line):
		...
		# if we're at the beginning of the line, pad with start sequences
